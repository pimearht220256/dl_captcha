...
def make_train_char_db(ori_img_dir):
    ...
    label = np.zeros((len(train_imgs),), dtype='int')
    index = 0
	...
	for i in line:
	   if len(str(letters.index(i))) == 1:
		label[index] = int(str(label[index]) + '0' + str(letters.index(i)))
	   else:
		label[index] = int(str(label[index]) + str(letters.index(i)))
	print label[index]
        index += 1
    ...
    file.close()
    caffe_train_path = os.path.join(os.getcwd(), 'captcha_train_all_lmdb')
...
def make_test_char_db(ori_img_dir):
    ...
    label = np.zeros((len(test_imgs),), dtype='int')
    file = open('5986.txt')
    answer_data = file.readlines()
    index = 0
    ...
    caffe_test_path = os.path.join(os.getcwd(), 'captcha_test_all_lmdb')
